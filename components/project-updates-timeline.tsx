  ...
  if (!updates.length) {
    return (
      <div className="card p-5 text-sm text-slate-600">
        Nenhuma atualização cadastrada para este projeto ainda.
      </div>
    );
  }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-slate-500">
                  {formatDateBR(item.update_date)} · Físico {item.physical_progress}% ·
                  Financeiro {item.financial_progress}% ·{" "}
                  {item.published_to_residents ? "Público" : "Interno"}
                </div>
              </div>
              <span
                className={`rounded-full px-3 py-1 text-xs font-medium ${
                  item.published_to_residents
