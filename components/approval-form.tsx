...
export function ApprovalForm({
  ...
  function setField<K extends keyof ApprovalFormValues>(key: K, value: ApprovalFormValues[K]) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="md:col-span-2">
            ...
            <input className={input} value={values.title} onChange={(e) => setField("title", e.target.value)} required />
          </div>
          <div>
            <label className={label}>Data</label>
            <input type="date" className={input} value={values.meeting_date} onChange={(e) => setField("meeting_date", e.target.value)} />
          </div>
          <div>
            <label className={label}>Decisão da assembleia</label>
            <select className={input} value={values.decision_status} onChange={(e) => setField("decision_status", e.target.value)}>
          ...
          <div>
            ...
            <select className={input} value={values.approval_type} onChange={(e) => setField("approval_type", e.target.value)}>
          ...
          <div>
            ...
            <select className={input} value={values.approval_status} onChange={(e) => setField("approval_status", e.target.value)}>
              <option value="pendente">Pendente</option>
              <option value="aprovado">Aprovado</option>
              <option value="reprovado">Reprovado</option>
            </select>
        ...
        <div>
          ...
          <textarea className={input + " min-h-24"} value={values.notes} onChange={(e) => setField("notes", e.target.value)} />
        </div>
        <div>
          <label className={label}>Comentários da aprovação</label>
          <textarea className={input + " min-h-24"} value={values.comments} onChange={(e) => setField("comments", e.target.value)} />
        </div>
