...
export function RfqForm({
  ...
  function setField<K extends keyof RfqFormValues>(key: K, value: RfqFormValues[K]) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input type="date" className={input} value={values.issue_date} onChange={(e) => setField("issue_date", e.target.value)} />
          </div>
          <div>
            <label className={label}>Prazo</label>
            <input type="date" className={input} value={values.due_date} onChange={(e) => setField("due_date", e.target.value)} />
          </div>
          <div>
            <label className={label}>Status</label>
            <select className={input} value={values.status} onChange={(e) => setField("status", e.target.value)}>
