      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <input type="date" className={input} value={values.update_date} onChange={(e) => setField("update_date", e.target.value)} required />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" step="0.01" className={input} value={values.physical_progress} onChange={(e) => setField("physical_progress", e.target.value)} />
          </div>
          <div>
            <label className={label}>Progresso financeiro (%)</label>
            <input type="number" step="0.01" className={input} value={values.financial_progress} onChange={(e) => setField("financial_progress", e.target.value)} />
          </div>
        </div>

        <div>
          <label className={label}>Resumo</label>
          <textarea className={input + " min-h-28"} value={values.summary} onChange={(e) => setField("summary", e.target.value)} placeholder="Descreva o que foi realizado, próximos passos, riscos e observações." />
        </div>

        <label className="flex items-center gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm">
          <input type="checkbox" checked={values.published_to_residents} onChange={(e) => setField("published_to_residents", e.target.checked)} />
